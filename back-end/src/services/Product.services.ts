...
class ProductServices {
  ...
  async deleteProduct(id: string): Promise<string> {
    try {
      const deleted_product = await ProductModel.deleteOne({
        _id: new ObjectId(id),
      });
      return "00";
    } catch (error) {
  ...
  async updateProduct(obj: IProduct): Promise<string> {
    // ket noi database de tao san pham
    try {
      const new_product = await ProductModel.updateOne(
  ...
  async viewDetailProduct(_id: string) {
    try {
      const result = ProductModel.find({ _id: new ObjectId(_id) });
